...
public class Engine extends Canvas implements Runnable {
    ...
    public static void setCurrentLevel(int level) {
        if (level == level1) {
            currentLevel = new Level01("/world/Map01-Forest.png", inputHandler);
        } else if (level == level2) {
            currentLevel = new Level02("/world/Map02-Dun.png", inputHandler);
        } else if (level == level3) {
            currentLevel = new Level03("/world/Map03-Dun.png", inputHandler);
        } else if (level == level4) {
            currentLevel = new Level04("/world/Map04-island.png", inputHandler);
        } else if (level == level5) {
            currentLevel = new Level05("/world/Map05-island.png", inputHandler);
        } else if (level == level6) {
            currentLevel = new Level06("/world/Map06-island.png", inputHandler);
        }
    }

    public static void setCurrentUI(int ui) {
        if (ui == mainMenuScreen) {
            currentUI = new MainMenu(screenWidth * scale, screenHeight * scale, inputHandler);
        } else if (ui == scoreScreen) {
            currentUI = new ScoreUI(screenWidth * scale, screenHeight * scale, inputHandler);
        } else if (ui == deathScreen) {
            currentUI = new DeathScreen(screenWidth * scale, screenHeight * scale, inputHandler);
        } else if (ui == winScreen) {
